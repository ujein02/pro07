					...
					<table class="table_form">
						<tbody>
							<tr>
								...
								<td><input type="text" name="user_id" id="user_id" size="100" class="single100" value="${member.user_id }" readonly required>
								</td>
							</tr>				
							<tr>
								<th><label for="userpw">비밀번호</label></th>
								<td><input type="password" name="user_pw" id="user_pw"  class="single100" value="${member.user_pw }" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" required>
									<p>(최소 8자리에서 최대 16자리까지, 숫자, 영문 대소문자, 특수문자가 각 1 문자 이상 포함되어야 함)</p>
								</td>
							</tr>
							<tr>
								<th><label for="userpw2">비밀번호 확인</label></th>
								<td><input type="password" name="user_pw2" id="user_pw2"  class="single100" value="${member.user_pw }" required></td>
							</tr>
							<tr>
								<th><label for="username">이름</label></th>
								<td><input type="text" name="user_name" id="user_name" class="single100"  value="${member.user_name }" required></td>
							</tr>
							<tr>
							...
							<tr>
								<td colspan="2">
									...
									<c:if test="${sid=='admin' }">
									<a href="${path1 }/member/delete.do?user_id=${member.user_id }" class="button btn-primary">직권 강퇴</a>
									</c:if>
									<c:if test="${sid!='admin' }">
